...
import { Github, ExternalLink, MessageSquare, Bug, Terminal, Edit } from 'lucide-react';
  ...
  if (!isLoaded) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="p-8 text-center text-gray-500">Loading your portfolio...</div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {apps.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-lg border-2 border-dashed border-gray-300">
            ...
            <div className="mt-6">
                <Link to="/new">
                    <Button variant="primary">Create New App</Button>
                </Link>
            </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {apps.map((app) => (
            <Card key={app.id} className="flex flex-col h-full hover:shadow-lg transition-shadow duration-200">
                <div className="relative h-48 bg-gray-200">
                    ...
                    <div className="absolute top-2 right-2 flex space-x-1">
                        {app.githubUrl && (
                            <a href={app.githubUrl} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-white/90 rounded-full hover:bg-white text-gray-800 transition">
                                <Github size={16} />
                            </a>
                        )}
                        {app.demoUrl && (
                            <a href={app.demoUrl} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-white/90 rounded-full hover:bg-white text-indigo-600 transition">
                ...
                <div className="flex-1 p-6 flex flex-col">
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <Link to={`/app/${app.id}/edit`} className="ml-2 p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition">
                            <Edit size={16} />
                        </Link>
                    </div>
                    ...
                    <div className="mt-4">
                        ...
                        <div className="flex gap-2">
                            <Link to={`/app/${app.id}/edit`} className="flex-1">
                                <Button variant="outline" className="w-full justify-center" icon={Edit}>
                                    Edit
                                </Button>
                            </Link>
